...
class OutfitGrid extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final profileController = ProfileSetupController();

    return FutureBuilder<String?>(
      future: profileController.loadGender(),
      builder: (context, genderSnapshot) {
        if (genderSnapshot.connectionState == ConnectionState.waiting) {
          return const Center(child: CircularProgressIndicator());
        }

        if (genderSnapshot.hasError || genderSnapshot.data == null) {
          return const Center(child: Text('Could not determine user gender.'));
        }

        final gender = genderSnapshot.data!;

        return FutureBuilder<List<Map<String, dynamic>>>(
          future: outfitService.retrieveSystemOutfitsByCategory(
            category: category,
            gender: gender,
          ),
              ...
              itemBuilder: (context, index) {
                ...
                return GestureDetector(
                  onTap: () {
                    showDialog(
                      context: context,
                      builder: (_) => SystemOutfitDisplay(systemOutfitName: outfitName),
                    );
                  },
                  child: Padding(
